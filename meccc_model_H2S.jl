...
H2S_model = DataFrame(Depth = 0:0.01:5)
H2S_model[!, :H2S] = twopart_concentration_profile.(1E-9, 5E-6, 1, 5, H2S_model[!, :Depth])
H2S_model[!, :H2S_µM] = H2S_model[!, :H2S] .* 1E6
...
conc_plot = plot(H2S_model, x = :H2S_µM, y = :Depth, 
    Geom.path(),
